                ...
                <ul className="grid gap-4 pt-5 md:grid-cols-2">
                    {guides.map((data) => (
                        <Link to={`/Guide/${data.name}`} state={data} key={data.id}>
                            <li className="relative overflow-hidden rounded-3xl bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md flex flex-col md:flex-row">
                                <div className="relative md:w-60 md:shrink-0">
                                    <div
                                    ...
                                    <img src={data.image} alt={data.name} className="h-60 w-full object-cover md:h-full" />
                                </div>
                                ...
                                <div className="flex flex-col gap-3 p-5">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {[data.hashtag_1, data.hashtag_2, data.hashtag_3].filter(Boolean).map((tag) => (
                                            <span key={tag} className="rounded-full border border-stone-300 px-3 py-1 text-sm text-[#938F8D]">{tag}</span>
                                        ))}
                                    </div>
                                </div>
                            </li>
