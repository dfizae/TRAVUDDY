...
export default function RecommendedSection() {
    ...
    return (
        <div className="px-4 mt-12.5 md:px-8">
            ...
            <ul className="grid gap-4 pt-5 md:grid-cols-2 xl:grid-cols-3">
                {guides.map((data) => (
                    <Link to={`/Guide/${data.name}`} state={data} key={data.id}>
                        <li className="relative overflow-hidden rounded-3xl bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
                            <div
                                onClick={(e) => toggleLike(e, data.id)}
                                className="absolute top-3 left-3 z-10 cursor-pointer rounded-full bg-white p-2 shadow-sm"
                            >
                                <img
                                    src={data.isLiked
                            ...
                            <img src={data.image} alt={data.name} className="h-60 w-full object-cover" />

                            <div className="flex flex-col gap-3 p-5">
                                <p className="font-semibold flex items-center gap-3 text-lg text-[#1A1A1A]">
                                    {data.name}
                                ...
                                <div className="flex flex-wrap gap-2">
                                    <span className="rounded-full border border-stone-300 px-3 py-1 text-sm text-[#938F8D]">{data.hashtag_1}</span>
                                    <span className="rounded-full border border-stone-300 px-3 py-1 text-sm text-[#938F8D]">{data.hashtag_2}</span>
                                </div>
                            </div>
                        </li>
                    </Link>
